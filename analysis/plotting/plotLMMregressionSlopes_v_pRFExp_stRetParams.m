...
if strcmp(modelToPlot,'CST_ST')
    ...
    title({'Group Average (+/- SEM): median pRF exponent vs suppression slope',...
        sprintf('CST_{opt} model: rho=%1.2f CI=[%1.2f,%1.2f] (p=%1.3f)',rho_ST(1,2),rhoLower_ST(1,2),rhoUpper_ST(1,2),rhoPval_ST(1,2)),...
        sprintf('CST_{fix} model: rho=%1.2f CI=[%1.2f,%1.2f] (p=%1.3f)',rho_Fix(1,2),rhoLower_Fix(1,2),rhoUpper_Fix(1,2),rhoPval_Fix(1,2))},...
        'FontSize',13);
else
    title({'Group Average (+/- SEM): median pRF exponent vs suppression slope',...
    sprintf('%s model: rho=%1.2f CI=[%1.2f,%1.2f] (p=%1.3f)',modelToPlot,...
