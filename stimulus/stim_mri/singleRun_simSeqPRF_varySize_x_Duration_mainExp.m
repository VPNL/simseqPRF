function singleRun_simSeqPRF_varySize_x_Duration_mainExp(subject,runNum,versionNr,loadTrials,offset,atScanner, coil, useEyelink)
% Wrapper function to run a single experimental run of the
% simultaneous-sequential spatiotemporal pRF experiment
%
% FOR USAGE BUTTON BOX at the scanner:
% set trigger box to these options (per JG):
% change modes / manual configure / HHSC 1X5D / USB / HID NAR NO5
%
% Nr of TRs: 389 (pilot 2)
% adapted from Poltoraski et al inverted faces experiment (2020)
%
% INPUTS
% [subject]       : (string) ID of subject (default to 'test')
% [runNum]        : (double) scan number (default to 999)
% [trials]        : (double) number specifying .mat file with trial ordering to present -
%                   useful if we need to match order etc across tasks/runs/subjects
% [offset]        : (1x2 vector) [x y] by which to offset screen center in
%                   units of pixels
% [atScanner]     : Used to switch keypress inputs, skip sync test or not
% [coil]          : Used to set screen, flip up down
% [useEyelink]    : Use eyetracker with calibration or not
...
if ~exist('runNum','var')
    scan.runNum = 999; else, scan.runNum  = runNum;
end

if ~exist('versionNr','var')
    scan.versionNr = 1; else, scan.versionNr  = versionNr;
end
...
if ~exist('atScanner','var')
    scan.trigger = 0; else, scan.trigger = atScanner; % set to zero to debug
end

if ~exist('coil','var')
    scan.coil = 16; else, scan.coil = coil;  % 32 or 16ch coil at CNI
end

if ~exist('useEyelink','var')
    scan.useEyelink = 0; else, scan.useEyelink = useEyelink;  % use eyetracker or not
end

%% Set up trigger box /  screen config depending on coil
if scan.coil == 16
    flipLR = 1;
    flipUD = 0;
%     if ~atScanner
...
scan.screenHeight      = 20; %104;              % in cm; % laptop=27.5, office=43, CNI = 104cm at both 16ch and 32ch
if scan.coil == 32
    scan.viewingDist = 43;                 % OLD: 272 cm;
else                                       % in cm; CNI = 270-273cm or 272 at 32ch, 265 at 16ch;
...
% ****** EYE TRACKER ******
% prompt for EDF file name and calibrate
if scan.useEyelink
    % Get edf file name
    edfName = [subject num2str(runNum) '_' datestr(now,'HHMM')];
    % Run calibration, validation, and drift correction
    setupEyelink_Bore(edfName, win);
end
% *************************
...
for n = 1:length(scan.allFlips)
    ...
    if n == 1
        ...
        if scan.trigger > 0
            while 1
                KbTriggerWait(KbName('t'), laptop); % check laptop to get trigger. or: checkKeyPress('t')
                [status, scan.time0] = newStartScan;
    ...
    % ****** EYE TRACKER ******
    if (scan.useEyelink && any(blockOnsets==n))
        Eyelink('message',sprintf('BLOCK_ID %d', mat2str(flip.block(n))));
    end
...
%% ****** EYE TRACKER ******
% Stop eyetracking and get edffile
if scan.useEyelink
    % Stop eyetracking
    Eyelink('message','TRIAL_END');
    Eyelink('StopRecording');
    cd(fullfile(simseqRootPath, 'data'))
...
save(fullfile(saveDir, ['simseqPRF_' scan.subj '_run' num2str(scan.runNum) '_v' num2str(versionNr) '_' scan.date '.mat']), 'params', 'scan', 'trial', 'flip', 'response','perf', '-v7.3');
